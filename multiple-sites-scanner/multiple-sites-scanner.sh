...
for row in $(cat servers.json | jq -r '.[] | @base64'); do
	...
	WP_VERSION=$(ssh -i $SSH_KEY_PATH $SSH_CONNECTION "wp core version --path=$SITE_DIRECTORY")
	PLUGINS=$(ssh -i $SSH_KEY_PATH $SSH_CONNECTION "wp plugin list --path=$SITE_DIRECTORY --fields=name,version --format=json")
	THEMES=$(ssh -i $SSH_KEY_PATH $SSH_CONNECTION "wp theme list --path=$SITE_DIRECTORY --fields=name,version --format=json")
